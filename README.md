...
6. 配置页和 Chat 使用跟随系统深浅色主题的界面；macOS 使用原生 traffic lights，Chat 只将标题栏安全区留给左侧 sidebar（折叠时用与交通灯同宽的标题栏顶盖融合，保持 DSH 原生折叠轨道和主聊天区不变），Windows/Linux 使用融合页面的悬浮窗口控制并为页面内容预留安全空间，不会跳转到系统默认浏览器。
