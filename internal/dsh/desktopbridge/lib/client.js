window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function hoverMessageActionsCSS() {
			...
			// Sticky shelf: above chat content (0–6) / with dock chrome (~10), below menus (70/100),
			// float (60), and Modal (1000). DSH 0.1.7-rc.1 has no --dsw-z-index-* tokens (hardcoded
			// layers); prefer --dsw-z-index-sticky if upstream adds it, else fall back to 10.
			// Was --dsw-z-index-popover/30 which covered 「本会话的审阅模型」 and similar dialogs.
			return [
								overlay + " { position: fixed !important; z-index: var(--dsw-z-index-sticky, 10) !important; box-sizing: border-box !important; display: flex !important; flex-direction: column !important; pointer-events: auto !important; opacity: 1 !important; outline: none !important; max-height: min(var(--dsh-desktop-prompt-overlay-avail, 100vh), calc(var(--dsh-desktop-prompt-overlay-lines, " + PROMPT_OVERLAY_DEFAULT_LINES + ") * " + PROMPT_OVERLAY_LINE_HEIGHT_REM + "rem + var(--dsh-desktop-prompt-overlay-extra, 0px) + " + PROMPT_OVERLAY_CHROME_PX + "px)) !important; }",
