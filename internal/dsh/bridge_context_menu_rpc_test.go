...
type rpcContextMenuHost struct {
	routeTestBridgeHost
	calls chan string
}

func (h *rpcContextMenuHost) ShowChatContextMenu(request ChatContextMenuRequest) error {
	url := request.Text
	h.calls <- url
	if url == "settings" {
...
func TestDesktopBridgeChatContextMenuPluginRPC(t *testing.T) {
	node, err := exec.LookPath("node")
	if err != nil {
		t.Skip("Node.js unavailable: skipping plugin RPC integration")
	}
	host := &rpcContextMenuHost{calls: make(chan string, 10)}
	owner := New()
...
async function request(method, payload, authenticated = true) {
 const response = await fetch("http://127.0.0.1:" + server.address().port + "/desktop-bridge/" + method, {
  ...
  body: JSON.stringify({ type: "client-request", rpcId: "context-menu-test", method, payload })
 });
...
try {
 const unauthorized = await request("showChatContextMenu", { x: 12, y: 34, text: "close", href: "https://example.com" }, false);
 assert.equal(unauthorized.status, 401);
 const valid = JSON.parse((await request("showChatContextMenu", { x: 12, y: 34, text: "close", href: "https://example.com" })).text);
 assert.deepEqual(valid, { type: "server-response", rpcId: "context-menu-test", result: { ok: true, value: { ok: true } } });
 const failed = JSON.parse((await request("showChatContextMenu", { x: 12, y: 34, text: "settings", href: "" })).text);
 assert.equal(failed.result.ok, false);
 assert.equal(failed.result.error.message, "launcher failed");
 assert.equal(failed.result.error.details.status, 409);
 const invalid = JSON.parse((await request("showChatContextMenu", { x: -1, y: 34, text: "invalid", href: "" })).text);
